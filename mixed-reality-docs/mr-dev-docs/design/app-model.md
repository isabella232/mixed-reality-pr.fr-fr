...
Les applications de réalité mixte créent une vue immersive, qui est obtenue avec le type [HolographicSpace](https://docs.microsoft.com/uwp/api/windows.graphics.holographic.holographicspace) .
...
Tout le stockage s’effectue par le biais de l' [espace de noms Windows. Storage](https://docs.microsoft.com/uwp/api/Windows.Storage). HoloLens ne prend pas en charge la synchronisation/l’itinérance du stockage d’application. Pour plus d’informations, consultez la documentation ci-dessous :
