...
- <span data-ttu-id="b4940-143">Un jeu de casque avec un microphone intégré (si le casque n’a pas de MIC et de haut-parleurs intégrés)</span><span class="sxs-lookup"><span data-stu-id="b4940-143">A set of headphones with a built-in microphone (if the headset doesn't have a built-in mic and speakers)</span></span>
...
## <a name="chapter-1--the-azure-portal"></a><span data-ttu-id="b4940-157">Chapitre 1 – portail Azure</span><span class="sxs-lookup"><span data-stu-id="b4940-157">Chapter 1 – The Azure Portal</span></span>
...
7. <span data-ttu-id="b4940-235">Dans ce volet, quelques paramètres doivent être vérifiés :</span><span class="sxs-lookup"><span data-stu-id="b4940-235">In this panel, a few settings need to be verified:</span></span>
    ...
    2. <span data-ttu-id="b4940-241">Dans l’onglet **paramètres de publication** , sous **fonctionnalités** , activez la case à cocher :</span><span class="sxs-lookup"><span data-stu-id="b4940-241">Within the **Publishing Settings** tab, under **Capabilities** , check:</span></span>
...
6.  <span data-ttu-id="b4940-287">Pour chaque **objet texte** , sélectionnez-le et utilisez les tableaux ci-dessous pour définir les paramètres dans le *panneau Inspecteur* .</span><span class="sxs-lookup"><span data-stu-id="b4940-287">For each **Text Object** , select it and use the below tables to set the parameters in the *Inspector Panel* .</span></span>
    ...
    2. <span data-ttu-id="b4940-309">Pour le composant **texte (script)** :</span><span class="sxs-lookup"><span data-stu-id="b4940-309">For the **Text (Script)** component:</span></span>
        ...
        |:----------------------:|:------------------:|:------------:|
        | <span data-ttu-id="b4940-313">MicrophoneStatusLabel</span><span class="sxs-lookup"><span data-stu-id="b4940-313">MicrophoneStatusLabel</span></span>  | <span data-ttu-id="b4940-314">État du microphone :</span><span class="sxs-lookup"><span data-stu-id="b4940-314">Microphone Status:</span></span> | <span data-ttu-id="b4940-315">20</span><span class="sxs-lookup"><span data-stu-id="b4940-315">20</span></span>           |
...
7.  <span data-ttu-id="b4940-420">Vous pouvez *supprimer* la méthode *Update ()* puisque cette classe ne l’utilise pas.</span><span class="sxs-lookup"><span data-stu-id="b4940-420">You can *delete* the *Update()* method since this class will not use it.</span></span>
...
<span data-ttu-id="b4940-435">Pour créer cette classe :</span><span class="sxs-lookup"><span data-stu-id="b4940-435">To create this Class:</span></span> 
...
1.  <span data-ttu-id="b4940-461">De retour dans l’éditeur Unity, cliquez et faites glisser la classe *results* *du* dossier **scripts** vers l’objet **Camera principal** dans le *panneau hiérarchie* .</span><span class="sxs-lookup"><span data-stu-id="b4940-461">Back in the Unity Editor, click and drag the *Results* class *from* the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel* .</span></span>
2.  <span data-ttu-id="b4940-462">Cliquez sur l' **appareil photo principal** et observez le panneau de l' *inspecteur* .</span><span class="sxs-lookup"><span data-stu-id="b4940-462">Click on the **Main Camera** and look at the *Inspector Panel* .</span></span> <span data-ttu-id="b4940-463">Vous remarquerez que dans le composant *script* nouvellement ajouté, il y a quatre champs avec des valeurs vides.</span><span class="sxs-lookup"><span data-stu-id="b4940-463">You will notice that within the newly added *Script* component, there are four fields with empty values.</span></span> <span data-ttu-id="b4940-464">Il s’agit des références de sortie des propriétés dans le code.</span><span class="sxs-lookup"><span data-stu-id="b4940-464">These are the output references to the properties in the code.</span></span> 
3.  <span data-ttu-id="b4940-465">Faites glisser les objets de **texte** appropriés du volet de la *hiérarchie* vers ces quatre emplacements, comme indiqué dans l’image ci-dessous.</span><span class="sxs-lookup"><span data-stu-id="b4940-465">Drag the appropriate **Text** objects from the *Hierarchy Panel* to those four slots, as shown in the image below.</span></span>

    ![Met à jour les références cibles avec les valeurs spécifiées.](images/AzureLabs-Lab1-34.png)
  
4.  <span data-ttu-id="b4940-467">Ensuite, cliquez et faites glisser la classe *Translator* du dossier **scripts** vers l’objet **Camera principal** dans le *panneau hiérarchie* .</span><span class="sxs-lookup"><span data-stu-id="b4940-467">Next, click and drag the *Translator* class from the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel* .</span></span> 
5.  <span data-ttu-id="b4940-468">Ensuite, cliquez et faites glisser la classe *MicrophoneManager* du dossier **scripts** vers l’objet **Camera principal** dans le *panneau hiérarchie* .</span><span class="sxs-lookup"><span data-stu-id="b4940-468">Then, click and drag the *MicrophoneManager* class from the **Scripts** folder to the **Main Camera** object in the *Hierarchy Panel* .</span></span> 
6.  <span data-ttu-id="b4940-469">Enfin, cliquez sur l' **appareil photo principal** et observez le panneau de l' *inspecteur* .</span><span class="sxs-lookup"><span data-stu-id="b4940-469">Lastly, click on the **Main Camera** and look at the *Inspector Panel* .</span></span> <span data-ttu-id="b4940-470">Vous remarquerez que dans le script que vous avez glissé, il existe deux zones de liste déroulante qui vous permettent de définir les langues.</span><span class="sxs-lookup"><span data-stu-id="b4940-470">You will notice that in the script you dragged on, there are two drop down boxes that will allow you to set the languages.</span></span>
 
    ![Vérifiez que les langues de traduction prévues sont entrées.](images/AzureLabs-Lab1-35.png)

## <a name="chapter-9--test-in-mixed-reality"></a><span data-ttu-id="b4940-472">Chapitre 9 – test en réalité mixte</span><span class="sxs-lookup"><span data-stu-id="b4940-472">Chapter 9 – Test in mixed reality</span></span>

<span data-ttu-id="b4940-473">À ce stade, vous devez vérifier que la scène a été correctement implémentée.</span><span class="sxs-lookup"><span data-stu-id="b4940-473">At this point you need to test that the Scene has been properly implemented.</span></span>

<span data-ttu-id="b4940-474">Assurez-vous que :</span><span class="sxs-lookup"><span data-stu-id="b4940-474">Ensure that:</span></span>

- <span data-ttu-id="b4940-475">Tous les paramètres mentionnés dans le [Chapitre 1](#chapter-1--the-azure-portal) sont correctement définis.</span><span class="sxs-lookup"><span data-stu-id="b4940-475">All the settings mentioned in [Chapter 1](#chapter-1--the-azure-portal) are set correctly.</span></span> 
- <span data-ttu-id="b4940-476">Les *résultats* , *Translator* et *MicrophoneManager* , les scripts sont attachés à l’objet **Camera principal** .</span><span class="sxs-lookup"><span data-stu-id="b4940-476">The *Results* , *Translator* , and *MicrophoneManager* , scripts are attached to the **Main Camera** object.</span></span> 
- <span data-ttu-id="b4940-477">Vous avez placé votre **clé** de Service *Azure API de traduction de texte Translator Text* dans la variable **AuthorizationKey** dans le script du *traducteur* .</span><span class="sxs-lookup"><span data-stu-id="b4940-477">You have placed your *Azure Translator Text API* Service **Key** within the **authorizationKey** variable within the *Translator* Script.</span></span>  
- <span data-ttu-id="b4940-478">Tous les champs du volet principal de l’inspecteur de l' *appareil photo* sont correctement affectés.</span><span class="sxs-lookup"><span data-stu-id="b4940-478">All the fields in the *Main Camera Inspector Panel* are assigned properly.</span></span>
- <span data-ttu-id="b4940-479">Votre microphone fonctionne lors de l’exécution de votre scène (dans le cas contraire, vérifiez que votre microphone attaché est l’appareil *par défaut* et que vous l’avez [configuré correctement dans Windows](https://support.microsoft.com/help/4027981/windows-how-to-set-up-and-test-microphones-in-windows-10)).</span><span class="sxs-lookup"><span data-stu-id="b4940-479">Your microphone is working when running your scene (if not, check that your attached microphone is the *default* device, and that you have [set it up correctly within Windows](https://support.microsoft.com/help/4027981/windows-how-to-set-up-and-test-microphones-in-windows-10)).</span></span>

<span data-ttu-id="b4940-480">Vous pouvez tester le casque immersif en appuyant sur le bouton **lecture** dans l' *éditeur Unity* .</span><span class="sxs-lookup"><span data-stu-id="b4940-480">You can test the immersive headset by pressing the **Play** button in the *Unity Editor* .</span></span>
<span data-ttu-id="b4940-481">L’application doit fonctionner par le biais du casque immersif attaché.</span><span class="sxs-lookup"><span data-stu-id="b4940-481">The App should be functioning through the attached immersive headset.</span></span>

> [!WARNING]  
> <span data-ttu-id="b4940-482">Si une erreur s’affiche dans la console Unity à propos du changement de périphérique audio par défaut, la scène peut ne pas fonctionner comme prévu.</span><span class="sxs-lookup"><span data-stu-id="b4940-482">If you see an error in the Unity console about the default audio device changing, the scene may not function as expected.</span></span> <span data-ttu-id="b4940-483">Cela est dû à la façon dont le portail de réalité mixte traite les microphones intégrés pour les casques.</span><span class="sxs-lookup"><span data-stu-id="b4940-483">This is due to the way the mixed reality portal deals with built-in microphones for headsets that have them.</span></span> <span data-ttu-id="b4940-484">Si vous voyez cette erreur, arrêtez simplement la scène et redémarrez-la et les choses devraient fonctionner comme prévu.</span><span class="sxs-lookup"><span data-stu-id="b4940-484">If you see this error, simply stop the scene and start it again and things should work as expected.</span></span>

## <a name="chapter-10--build-the-uwp-solution-and-sideload-on-local-machine"></a><span data-ttu-id="b4940-485">Chapitre 10 : créer la solution UWP et chargement sur l’ordinateur local</span><span class="sxs-lookup"><span data-stu-id="b4940-485">Chapter 10 – Build the UWP solution and sideload on local machine</span></span>

<span data-ttu-id="b4940-486">Tout ce qui est nécessaire pour la section Unity de ce projet est maintenant terminé. il est donc temps de la générer à partir d’Unity.</span><span class="sxs-lookup"><span data-stu-id="b4940-486">Everything needed for the Unity section of this project has now been completed, so it is time to build it from Unity.</span></span>

1.  <span data-ttu-id="b4940-487">Accédez à **paramètres de build** : **fichier > paramètres de Build...**</span><span class="sxs-lookup"><span data-stu-id="b4940-487">Navigate to **Build Settings** : **File > Build Settings...**</span></span>
2.  <span data-ttu-id="b4940-488">Dans la fenêtre **paramètres de build** , cliquez sur **générer** .</span><span class="sxs-lookup"><span data-stu-id="b4940-488">From the **Build Settings** window, click **Build** .</span></span>

    ![Générez la scène Unity.](images/AzureLabs-Lab1-36.png)
  
3.  <span data-ttu-id="b4940-490">Si ce n’est pas déjà le cas, Tick **Unity C# Projects** .</span><span class="sxs-lookup"><span data-stu-id="b4940-490">If not already, tick **Unity C# Projects** .</span></span>
4.  <span data-ttu-id="b4940-491">Cliquez sur **Générer** .</span><span class="sxs-lookup"><span data-stu-id="b4940-491">Click **Build** .</span></span> <span data-ttu-id="b4940-492">Unity lance une fenêtre de l' *Explorateur de fichiers* , dans laquelle vous devez créer, puis sélectionner un dossier dans lequel générer l’application.</span><span class="sxs-lookup"><span data-stu-id="b4940-492">Unity will launch a *File Explorer* window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="b4940-493">Créez ce dossier maintenant, puis nommez-le *application* .</span><span class="sxs-lookup"><span data-stu-id="b4940-493">Create that folder now, and name it *App* .</span></span> <span data-ttu-id="b4940-494">Ensuite, avec le dossier d' *application* sélectionné, appuyez sur **Sélectionner un dossier** .</span><span class="sxs-lookup"><span data-stu-id="b4940-494">Then with the *App* folder selected, press **Select Folder** .</span></span> 
5.  <span data-ttu-id="b4940-495">Unity commence à générer votre projet dans le dossier de l' *application* .</span><span class="sxs-lookup"><span data-stu-id="b4940-495">Unity will begin building your project to the *App* folder.</span></span> 
6.  <span data-ttu-id="b4940-496">Une fois la génération de Unity terminée (cela peut prendre un certain temps), une fenêtre de l' *Explorateur de fichiers* s’ouvre à l’emplacement de votre Build (Vérifiez la barre des tâches, car elle ne s’affiche pas toujours au-dessus de votre Windows, mais vous informera de l’ajout d’une nouvelle fenêtre).</span><span class="sxs-lookup"><span data-stu-id="b4940-496">Once Unity has finished building (it might take some time), it will open a *File Explorer* window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-11--deploy-your-application"></a><span data-ttu-id="b4940-497">Chapitre 11 – déployer votre application</span><span class="sxs-lookup"><span data-stu-id="b4940-497">Chapter 11 – Deploy your application</span></span>

<span data-ttu-id="b4940-498">Pour déployer votre application :</span><span class="sxs-lookup"><span data-stu-id="b4940-498">To deploy your application:</span></span>

1.  <span data-ttu-id="b4940-499">Accédez à votre nouvelle build Unity (le dossier de l' *application* ) et ouvrez le fichier solution avec *Visual Studio* .</span><span class="sxs-lookup"><span data-stu-id="b4940-499">Navigate to your new Unity build (the *App* folder) and open the solution file with *Visual Studio* .</span></span>
2.  <span data-ttu-id="b4940-500">Dans la configuration de la solution, sélectionnez **Déboguer** .</span><span class="sxs-lookup"><span data-stu-id="b4940-500">In the Solution Configuration select **Debug** .</span></span>
3.  <span data-ttu-id="b4940-501">Dans la plateforme de la solution, sélectionnez **x86** , **ordinateur local** .</span><span class="sxs-lookup"><span data-stu-id="b4940-501">In the Solution Platform, select **x86** , **Local Machine** .</span></span> 

    > <span data-ttu-id="b4940-502">Pour Microsoft HoloLens, il peut s’avérer plus facile de définir cette valeur sur *machine distante* , afin de ne pas être attaché à votre ordinateur.</span><span class="sxs-lookup"><span data-stu-id="b4940-502">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine* , so that you are not tethered to your computer.</span></span> <span data-ttu-id="b4940-503">Toutefois, vous devez également effectuer les opérations suivantes :</span><span class="sxs-lookup"><span data-stu-id="b4940-503">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="b4940-504">Identifiez l' **adresse IP** de votre HoloLens, qui se trouve dans les *paramètres > réseau & Internet > les options avancées du Wi-Fi >* ; IPv4 est l’adresse que vous devez utiliser.</span><span class="sxs-lookup"><span data-stu-id="b4940-504">Know the **IP Address** of your HoloLens, which can be found within the *Settings > Network & Internet > Wi-Fi > Advanced Options* ; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="b4940-505">Assurez-vous que le *mode développeur* est **activé** ; trouvé dans *paramètres > mettre à jour & > de sécurité pour les développeurs* .</span><span class="sxs-lookup"><span data-stu-id="b4940-505">Ensure *Developer Mode* is **On** ; found in *Settings > Update & Security > For developers* .</span></span>

    ![Déployez la solution à partir de Visual Studio.](images/AzureLabs-Lab1-37.png)
    
 
4.  <span data-ttu-id="b4940-507">Accédez au **menu Générer** , puis cliquez sur **déployer la solution** pour chargement l’application sur votre PC.</span><span class="sxs-lookup"><span data-stu-id="b4940-507">Go to **Build menu** and click on **Deploy Solution** to sideload the application to your PC.</span></span>
5.  <span data-ttu-id="b4940-508">Votre application doit maintenant apparaître dans la liste des applications installées, prêtes à être lancées.</span><span class="sxs-lookup"><span data-stu-id="b4940-508">Your App should now appear in the list of installed apps, ready to be launched.</span></span>
6.  <span data-ttu-id="b4940-509">Une fois lancé, l’application vous invite à autoriser l’accès au microphone.</span><span class="sxs-lookup"><span data-stu-id="b4940-509">Once launched, the App will prompt you to authorize access to the Microphone.</span></span> <span data-ttu-id="b4940-510">Veillez à cliquer sur le bouton **Oui** .</span><span class="sxs-lookup"><span data-stu-id="b4940-510">Make sure to click the **YES** button.</span></span>
7.  <span data-ttu-id="b4940-511">Vous êtes maintenant prêt à commencer la traduction.</span><span class="sxs-lookup"><span data-stu-id="b4940-511">You are now ready to start translating!</span></span>

## <a name="your-finished-translation-text-api-application"></a><span data-ttu-id="b4940-512">Votre application API de texte de traduction terminé</span><span class="sxs-lookup"><span data-stu-id="b4940-512">Your finished Translation Text API application</span></span>

<span data-ttu-id="b4940-513">Félicitations, vous avez créé une application de réalité mixte qui tire parti de l’API de traduction de texte Azure pour convertir la parole en texte traduit.</span><span class="sxs-lookup"><span data-stu-id="b4940-513">Congratulations, you built a mixed reality app that leverages the Azure Translation Text API to convert speech to translated text.</span></span>

![Produit final.](images/AzureLabs-Lab1-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="b4940-515">Exercices bonus</span><span class="sxs-lookup"><span data-stu-id="b4940-515">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="b4940-516">Exercice 1</span><span class="sxs-lookup"><span data-stu-id="b4940-516">Exercise 1</span></span>

<span data-ttu-id="b4940-517">Pouvez-vous ajouter la fonctionnalité de conversion de texte par synthèse vocale à l’application, afin que le texte retourné soit parlé ?</span><span class="sxs-lookup"><span data-stu-id="b4940-517">Can you add text-to-speech functionality to the app, so that the returned text is spoken?</span></span>

### <a name="exercise-2"></a><span data-ttu-id="b4940-518">Exercice 2</span><span class="sxs-lookup"><span data-stu-id="b4940-518">Exercise 2</span></span>

<span data-ttu-id="b4940-519">Permet à l’utilisateur de modifier les langues source et de sortie (« from » et « to ») au sein de l’application elle-même, de sorte que l’application n’a pas besoin d’être reconstruite chaque fois que vous souhaitez modifier des langues.</span><span class="sxs-lookup"><span data-stu-id="b4940-519">Make it possible for the user to change the source and output languages ('from' and 'to') within the app itself, so the app does not need to be rebuilt every time you want to change languages.</span></span>
