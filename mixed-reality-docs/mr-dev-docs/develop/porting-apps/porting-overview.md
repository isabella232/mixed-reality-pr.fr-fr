...
Tout d’abord, configurez votre projet cible en vous basant sur nos recommandations [Unity](#unity) et [inréelles](#unreal) , puis suivez un ou plusieurs de nos scénarios de Portage :
...
Il est important de maintenir à jour vos projets, que vous portiez ou non une application vers un autre appareil cible. Reportez-vous aux ressources basées sur le moteur listées ci-dessous pour obtenir nos recommandations actuelles.
...
Si vous disposez d’une application UWP 2D existante que vous souhaitez porter vers un casque ou HoloLens Windows Mixed Reality, suivez les instructions de notre article Portage d' [applications UWP 2D pour Windows Mixed Reality](building-2d-apps.md) .
