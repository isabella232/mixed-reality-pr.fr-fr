...
Les documents Windows Mixed Reality se trouvent désormais sur la plateforme docs.microsoft.com, qui utilise la démarque GitHub (avec les fonctionnalités Markdig). Fondamentalement, le contenu que vous modifiez dans ce référentiel est converti en pages mises en forme et stylisées qui s’affichent dans https://docs.microsoft.com/windows/mixed-reality . 

Cette page présente les étapes et les instructions de base pour la contribution, ainsi que des liens vers les concepts de base de la marque. Merci beaucoup pour votre contribution.
...
- `redirect_url`Est l’URL publique relative de l’ancien article vers le nouvel article. Assurez-vous que cette URL **ne contient pas** `mixed-reality-docs` ou `.md` , car elle fait référence à l’URL publique et non au chemin d’accès au référentiel. La liaison à une section dans le nouvel article à l’aide de `#section` est autorisée. Vous pouvez également utiliser un chemin d’accès absolu à un autre site, si nécessaire.
